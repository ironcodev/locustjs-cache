...
class CacheItem {
  ...
  setValue(value, duration) {
    ...
    this.hits = 0;
    this.duration = (0, _base.isNumeric)(duration) ? parseInt(duration) : this.duration;
    if (isNaN(this.duration) || (0, _base.isEmpty)(this.duration)) {
      this.duration = 0;
    }
  }
  hit() {
...
class CacheBase {
  constructor(config) {
    (0, _exception.throwIfInstantiateAbstract)(CacheBase, this);
  ...
  getDuration(duration) {
    let result = 0;
    if ((0, _base.isNumeric)(duration)) {
      result = parseInt(duration);
    } else {
      if (this.config && (0, _base.isNumeric)(this.config.duration)) {
        result = parseInt(this.config.duration);
      }
    }
    if (isNaN(result) || (0, _base.isEmpty)(result)) {
      result = 0;
    }
    return result;
  }
...
class CacheDefault extends CacheBase {
  ...
  setItemAsync(key, value, duration) {
    ...
    if (entry == null) {
      ...
      if ((0, _base.isFunction)(value)) {
        ...
        if (_value && (0, _base.isFunction)(_value.then)) {
          result = new Promise((resolve, reject) => {
            _value.then(r => {
  ...
  contains(value, fnEqualityComparer) {
    if (!(0, _base.isFunction)(fnEqualityComparer)) {
